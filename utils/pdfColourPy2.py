...
def cmyk_per_page(pdf_file):
    ...
    gs_inkcov = subprocess.Popen(
        ["gs", "-o", "-", "-sDEVICE=inkcov", pdf_file], stdout=subprocess.PIPE
    )
    ...
    for raw_line in iter(gs_inkcov.stdout.readline, b""):
        ...
        fields = line.split()
        if len(fields) >= CMYK_NCOLORS and all(
            RE_FLOAT.match(fields[i]) for i in range(CMYK_NCOLORS)
        ):
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="""Lists or counts the
    ...
    parser.add_argument(
        "--debug", "-d", action="store_true", help="Enables verbose debugging output"
    )
    parser.add_argument(
        "--noheader", "-H", action="store_true", help="Disables the first header line"
    )
    parser.add_argument(
        "--pcolor",
        "-C",
        metavar="PC",
        type=float,
        help="Color page price (for total cost report, " + "requires --pblack as well)",
    )
    parser.add_argument(
        "--pblack",
        "-B",
        metavar="PB",
        type=float,
        help="B/W page price (for total cost report, " + "requires --pcolor as well)",
    )
    ...
    if args.pcolor is not None and args.pblack is None:
        raise Exception("Page price was specified for color but not for B/W pages")
    if args.pblack is not None and args.pcolor is None:
        raise Exception("Page price was specified for B/W but not for color pages")
    ...
    if args.count:
        ...
        print(
            (
                "Total cost ({0:d} B/W @ {1:3.6g}/page "
